    ...
    def create_summary_page(
        ...
        payload = {
            ...
            "properties": {
                ...
                "檔案名稱":    {"rich_text":  [{"text": {"content": file_name}}]},
                "資料夾":      {"rich_text":  [{"text": {"content": folder_name}}]},
...
def process_one_file(
    ...
    try:
        ...
        print(f"      ❌ 失敗：{exc}")
        return {"ok": False, "file_name": file_name, "error": str(exc)}
...
def main():
    ...
    print(f"\n{'─' * 50}")
    results = [
        process_one_file(fi, drive, transcriber, summarizer, notion, github, tracker)
        for fi in new_files
    ]
